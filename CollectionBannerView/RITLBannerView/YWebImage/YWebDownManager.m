...
//    NSLog(@"开始下载图片啦,路径为:%@",imagePath);
    ...
    if ([[YWebFileManager shareInstance] fileIsExist:imagePath])
    {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            //调用下载成功的回调
            if(weakSelf.finishBlockHandle)
            {
                NSLog(@"本地图片!");
                weakSelf.finishBlockHandle(path);
            }
...
-(void)pauseDown
{
    ...
    [self.downLoadTask cancelByProducingResumeData:^(NSData * _Nullable resumeData) {
        ...
        //如果下载url存在
        if(weakSelf.imageURL)
        {
            //需要将未下载完毕的数据以字典形式存到UserDefault
            [[NSUserDefaults standardUserDefaults] setValue:resumeData forKey:weakSelf.imageName];
        }
    }];
...
totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        //执行过程的回调
        if (weakSelf.progressBlockHandle) {
            weakSelf.progressBlockHandle(bytesWritten,totalBytesWritten,totalBytesExpectedToWrite);
        }
    });
}
...
#else
    return [YWebDataHandle stringForBase64Handle:_imageURL.absoluteString];
#endif
}
