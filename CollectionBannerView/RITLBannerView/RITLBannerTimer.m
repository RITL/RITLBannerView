...
@implementation RITLBannerTimer
...
{
    //获得block对象
    RITLBannerTimerBlock block = userInfo.lastObject;
...
+(NSTimer *)scheduledTimerWithTimeInterval:(NSTimeInterval)timeInterval target:(id)aTarget selector:(SEL)aSelector userInfo:(id)userInfo repeats:(BOOL)isRepeats
{
    ...
    RITLBannerTimer * timerObject = [RITLBannerTimer bannerViewTimer:aTarget selector:aSelector];
...
+(NSTimer *)scheduledTimerWithTimeInterval:(NSTimeInterval)timeInterval userInfo:(id)userInfo repeats:(BOOL)isRepeats BlockHandle:(RITLBannerTimerBlock)blockHandle
{
