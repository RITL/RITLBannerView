...
typedef void(^RITLBannerTimerBlock)(id info);


@interface RITLBannerTimer : NSObject
...
/** 获得一个计时器，Block执行 */
+ (NSTimer *)scheduledTimerWithTimeInterval:(NSTimeInterval)timeInterval userInfo:(id __nullable)userInfo repeats:(BOOL)isRepeats BlockHandle:(RITLBannerTimerBlock)blockHandle;
